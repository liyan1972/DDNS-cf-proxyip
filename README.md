...
```text
直接在页面中输入AUTH_KEY的值或直接访问https://你的-worker-url/?key=你的AUTH_KEY
```
...
1. 维护的域名配置：维护的域名，对应的Zone ID、CF Key。
...
> 另外说明，点击从**库中移除**则从库中移除输入框中的数据。比如想清空ip库，可以从库中加载所有的ip至输入框，点击从库中移除则库中所有的ip就都清空了。
